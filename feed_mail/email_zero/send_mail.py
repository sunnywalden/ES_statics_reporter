...
from zero_frist_type_day import DayFristTypeClickRate
...
class sendMail:
    def __init__(self):
        ...
        self.subject = "统计结果：%s-feed-0元包统计" % self.day  # 邮件标题
    ...
    def http_post(self, html):
        ...
        data = {
            ...
            "emailsubject": {
                "mess": html,
                "subject": "统计结果：" + self.day + "feed-0元包统计"
            }
        }
        print data
    ...
    def frist_type(self, html):
        result_one, result_two = DayFristTypeClickRate().feed_handle()
        for key, value in result_one.items():
            if key != self.key:
                exposure, click, user, rate = value[0], value[1], result_two[key], value[2]
                rowspan = len(exposure) + 2
                feed_titile = '<tr><td rowspan=%s>0元包一级分类</td><td></td><td>曝光量</td><td>点击量</td><td>去重用户</td><td>点击率</td></tr>' % rowspan
                html += feed_titile
                for k, v in exposure.items():
                    html += '<tr><td>' + k + '</td><td>' + str(exposure[k]) + '</td><td>' + str(
                        click[k]) + '</td><td>' + str(user[k]) + '</td><td>' + rate[k] + '</td></tr>'
                exprose_sum = sum(exposure.itervalues())
                click_sum = sum(click.itervalues())
                if float(exprose_sum) != float(0):
                    html += '<tr><td>总计</td><td>' + str(exprose_sum) + '</td><td>' + str(click_sum) + '</td><td>' + str(
