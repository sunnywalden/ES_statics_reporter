...
#                                        break
    ...
    def request_es(self, qsl_body):
        self.client.es_conn.bulk(index=self.index, doc_type=self.type, body=qsl_body, request_timeout=500)
...
def log_handler(year, month, day1, day2):
    pool = threadpool.ThreadPool(30)
    days = reversed(range(day1, day2))
    timestamps = []
    for day in days:
	for hour in range(0, 24):
        	new_time = (year, month, day, hour, 0, 0, 0, 0, 0)
        	timestamp = time.localtime(time.mktime(new_time))
        	timestamps.append(timestamp)
    # handler = HandleFeed(timestamps)
    # handler.create_index()
    # handler.handle_feed()
    handers = threadpool.makeRequests(HandleFeed, timestamps, index_general)
    for hander in handers:
        pool.putRequest(hander)
    pool.wait()

def index_general(request_id, handler):
        handler.create_index()
        handler.handle_feed()
...
if __name__ == '__main__':
    ...
    #for day in reversed(days):
        for hour in range(0, 24):
            new_time = (2018, 8, 9, hour, 0, 0, 0, 0, 0)
            timestamp = time.localtime(time.mktime(new_time))
    ...
    #log_handler(2018, 8, 1, 12)
#    log_handler(2018, 8, 1, 10)
